...
games_df = pd.read_csv("../DB/add6month.csv",index_col=0)
games_df["raceID"] = games_df["raceID"].map(lambda x: str(x)[:-2])
games_df["raceID"] = games_df["raceID"].astype("int64")

games_df["moterNo"] = games_df["jcd"].map(lambda x: str(int(x)).zfill(2)) + games_df["moterNo"].map(lambda x: str(int(x)).zfill(2))

racer_df = games_df.drop_duplicates("moterNo")
#racer_df = racer_df["moterNo"]

env = trueskill.TrueSkill(beta=6.25,backend="mpmath")

ratings = {}
for i in racer_df["moterNo"]:
    if (i in ratings) == False:
        ratings[i] = env.create_rating()
...
win_prob = np.reshape(np.array(win_prob),[-1,6])
rating_df = pd.DataFrame(data={"moter_mu":mu,"motetr_sigma":sigma,"moter_exp":exp,"moter_quality":quality},index=indexs)
prob_df =pd.DataFrame(win_prob,columns=["moter_1","moter_2","moter_3","moter_4","moter_5","moter_6"]).astype(float)

rateing_df = rating_df.sort_index()
prob_df = prob_df.sort_index()
X = pd.concat([rateing_df,prob_df],axis=1)
X.to_csv("fixmotorrate.csv",mode="w",encoding="utf-8-sig")
sys.exit()
